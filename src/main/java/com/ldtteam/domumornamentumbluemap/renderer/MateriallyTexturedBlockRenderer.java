package com.ldtteam.domumornamentumbluemap.renderer;
 ...
 * <p>
 * This is a full copy of the {@link de.bluecolored.bluemap.core.map.hires.block.ResourceModelRenderer}, with the addition of Domum support.
 */
public class MateriallyTexturedBlockRenderer implements BlockRenderer
{
    ...
    public MateriallyTexturedBlockRenderer(ResourcePack resourcePack, TextureGallery textureGallery, RenderSettings renderSettings)
    {
    ...
    private void buildModelElementResource(Element element, TileModelView blockModel)
    {
        ...
        float minX = Math.min(from.getX(), to.getX()), minY = Math.min(from.getY(), to.getY()), minZ = Math.min(from.getZ(), to.getZ()), maxX = Math.max(from.getX(), to.getX()),
            maxY = Math.max(from.getY(), to.getY()), maxZ = Math.max(from.getZ(), to.getZ());
        ...
        blockModel.transform(modelElementTransform.copy(element.getRotation().getMatrix()).scale(BLOCK_SCALE, BLOCK_SCALE, BLOCK_SCALE));
    }
    ...
    private void createElementFace(Element element, Direction faceDir, VectorM3f c0, VectorM3f c1, VectorM3f c2, VectorM3f c3)
    {
        ...
        if (block.isRemoveIfCave() && (renderSettings.isCaveDetectionUsesBlockLight() ? Math.max(blockLight, sunLight) : sunLight) == 0)
        {
            return;
        }

        // calculate faceRotationVector
        faceRotationVector.set(faceDirVector.getX(), faceDirVector.getY(), faceDirVector.getZ());
        faceRotationVector.rotateAndScale(element.getRotation().getMatrix());
        makeRotationRelative(faceRotationVector);

        // face culling
        if (renderSettings.isRenderTopOnly() && faceRotationVector.y < 0.01)
        {
            return;
        }
        ...
        // ####### texture
        ResourcePath<Texture> texturePath = getDomumMappedTexture(face.getTexture().getTexturePath(modelResource.getTextures()::get), faceDir);
        ...
        if (variant.isUvlock() && variant.isTransformed())
        {
            ...
            uvRotation = variant.getY() * (faceDirVector.getY() * xRotCos + faceDirVector.getZ() * xRotSin) + variant.getX() * (1 - faceDirVector.getY());
        }
    ...
    private ExtendedBlock getRotationRelativeBlock(int dx, int dy, int dz)
    {
        ...
        return block.getNeighborBlock(Math.round(rotationRelativeBlockDirection.x), Math.round(rotationRelativeBlockDirection.y), Math.round(rotationRelativeBlockDirection.z));
    }
    ...
    private float testAo(VectorM3f vertex, Direction dir)
    {
        ...
        {
            if (getRotationRelativeBlock(x, y, 0).getProperties().isOccluding())
            {
                occluding++;
            }
        }
        ...
        {
            if (getRotationRelativeBlock(x, 0, z).getProperties().isOccluding())
            {
                occluding++;
            }
        }
        ...
        {
            if (getRotationRelativeBlock(0, y, z).getProperties().isOccluding())
            {
                occluding++;
            }
        }
        ...
        {
            if (getRotationRelativeBlock(x, y, z).getProperties().isOccluding())
            {
                occluding++;
            }
        }
    ...
    private ResourcePath<Texture> getDomumMappedTexture(final ResourcePath<Texture> original, final Direction faceDir)
    {
        if (original != null)
        {
            ...
            if (blockEntity instanceof MateriallyTexturedBlockEntity materiallyTexturedBlockEntity)
            {
                ...
                if (!replacementBlock.isBlank())
                {
                    ...
                    {
                        final Model mappedBlockModel = resourcePack.getModel(variant.get().getModel());
